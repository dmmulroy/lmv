...
function sortChildren(nodes: TreeNode[], sortOrder: SortOrder) {
  const byNameAsc = (a: TreeNode, b: TreeNode) => compareStrings(a.name, b.name);
  const byNameDesc = (a: TreeNode, b: TreeNode) => -compareStrings(a.name, b.name);
...
export function filterTree(
  ...
  const matchesFile = (path: string) => normalizePath(path).toLowerCase().includes(q);
...
export function flattenVisibleNodes(
  ...
  const walk = (node: TreeNode, depth: number) => {
    out.push({ node, depth });
    if (node.kind === "folder") {
      const expanded = expandedFolders.has(node.path) || autoExpand.has(node.path);
      if (!expanded) return;
