...
export function App() {
  ...
  useEffect(() => {
    ...
    if (storedWidth) {
      ...
      if (Number.isFinite(n)) setSidebarWidthPct(Math.min(0.6, Math.max(0.15, n)));
    }

    const storedSort = localStorage.getItem("lmv-sort-order") as SortOrder | null;
    if (storedSort) setSortOrder(storedSort);

    fetch("/api/files")
      .then((res) => res.json())
      .then((data: { files: ApiFile[]; singleFile: boolean; pendingRefresh?: boolean }) => {
        setFiles(data.files || []);
